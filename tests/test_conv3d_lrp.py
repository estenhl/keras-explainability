...
# def test_conv3d_lrp_alpha_1_beta_0():
#     input = Input((4, 4, 4, 1))
#     x = Conv3D(2, (3, 3, 3), activation=None, padding='SAME')(input)
#     x = Flatten()(x)
...
#     assert np.isclose(0.14932, explanations[0,0,0,1,0], 1e-5), \
#         'Conv3D with alpha=1, beta=0 returns the wrong explanations'
#     assert np.isclose(5.22624, explanations[0,0,1,1,0], 1e-5), \
#         'Conv3D with alpha=1, beta=0 returns the wrong explanations'
...
#     assert np.isclose(132, np.sum(explanations)), \
#         'Conv3D with alpha=2, beta=1 returns the wrong explanations'
#     assert np.isclose(0.29864, explanations[0,0,0,1,0], 1e-5), \
#         'Conv3D with alpha=2, beta=1 returns the wrong explanations'
#     assert np.isclose(-3.51063, explanations[0,0,1,0,0], 1e-5), \
#         'Conv3D with alpha=2, beta=1 returns the wrong explanations'
#     assert np.isclose(10.45248, explanations[0,0,1,1,0], 1e-5), \
#         'Conv3D with alpha=2, beta=1 returns the wrong explanations'
#     assert np.isclose(-47.74468, explanations[0,1,0,0,0], 1e-5), \
#         'Conv3D with alpha=2, beta=1 returns the wrong explanations'
#     assert np.isclose(96.46154, explanations[0,1,0,1,0], 1e-5), \
#         'Conv3D with alpha=2, beta=1 returns the wrong explanations'
#     assert np.isclose(-80.74467, explanations[0,1,1,0,0], 1e-5), \
#         'Conv3D with alpha=2, beta=1 returns the wrong explanations'
#     assert np.isclose(156.78734, explanations[0,1,1,1,0], 1e-5), \
#         'Conv3D with alpha=2, beta=1 returns the wrong explanations'

def test_conv3d_lrp_multiple():
    np.random.seed(42)

    inputs = Input((4, 4, 4, 2))
    conv = Conv3D(2, (3, 3, 3), padding='SAME')(inputs)
    model = Model(inputs, conv)

    for _ in range(100):

        model.layers[-1].set_weights([
            np.random.uniform(-1, 1, (3, 3, 3, 2, 2)),
            np.ones(2)
        ])

        data = np.random.uniform(-1, 1, (1, 4, 4, 4, 2))
        predictions = model.predict(data)
        #mask = np.zeros_like(predictions)
        #mask[0,0,0,0] = predictions[0,0,0,0]
        explainer = Conv3DLRP(model.layers[-1])
        explanations = explainer([data, predictions])

        print(np.sum(predictions))
        print(np.sum(explanations))
